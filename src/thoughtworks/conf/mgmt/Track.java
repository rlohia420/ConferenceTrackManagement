...
public class Track {
    ...
    static Optional<Talk> FindTalkToSchedule(List<Talk> unscheduledTalks, Session session) {
        ...
        final Comparator<Talk> byAscendingSessionAvailableTimeModuloTalkDuration = Comparator.comparing(
                (Talk unscheduledTalk) -> session.timeAvailable().toMinutes() % unscheduledTalk.talkDuration.toMinutes());
        final Comparator<Talk> byDescendingTalkByDuration = Comparator.comparingLong((Talk unscheduledTalk) -> unscheduledTalk.talkDuration.toMinutes()).reversed();
